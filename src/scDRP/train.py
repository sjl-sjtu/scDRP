...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model):
        """
        Call the early stopping mechanism.
...
def train_model(device, writer, train_dataset, valid_dataset, model, epoch_num, batch_size,
    ...
    """
    # load data
    train_data = DataLoader(train_dataset,batch_size,shuffle=True,drop_last=True,num_workers=6,pin_memory=True)
    ...
    for epoch in range(epoch_num):
        model.train()
        total_loss,recon_loss,kl_loss,ind_loss,l1_norm,l0_latent = 0,0,0,0,0,0
        # total_samples = 0
        for step, (x,a,t,c) in enumerate(train_data):
            model.train()
            x,a,t,c = x.to(device),a.to(device),t.to(device),c.to(device)
            ...
            if count_data:
                _,_,_,_, _, _, _, _, _, _, loss, loss_dict = model(x,a,t,c)
            else:
                _,_,_,_, _, _, _, loss, loss_dict = model(x,a,t,c)
            ...
            if writer is not None:
                ...
                writer.add_scalar("L1_latent/train", loss_dict['l0_latent'], epoch*num_batch+step+1)
        ...
        if writer is not None:    
            ...
            writer.add_scalar("L1_latent_epoch/train", l0_latent/num_batch, epoch+1)

        if (epoch + 1) % 10 == 0:
            print("epoch {}: loss = {:.4f}, recon_loss = {:.4f}, kl_loss = {:.4f}, ind_loss = {:.4f}, l1_norm = {:.4f}, l0_latent = {:.4f}".format(
                epoch+1,total_loss/num_batch,recon_loss/num_batch,kl_loss/num_batch,ind_loss/num_batch,l1_norm/num_batch,l0_latent/num_batch))
        # if (epoch + 1) % 10 == 0:
        #     print("epoch {}: loss = {:.4f}, recon_loss = {:.4f}, kl_loss = {:.4f}, l1_norm = {:.4f}, l0_latent = {:.4f}".format(
        #         epoch+1,total_loss/num_batch,recon_loss/num_batch,kl_loss/num_batch,l1_norm/num_batch,l0_latent/num_batch))
    ...
    # zd,zu,rho = inference_model(device, train_dataset, model, batch_size)
    # return zd,zu,rho
...
def validate_model(device, validate_dataset, model, batch_size, count_data):
    ...
    """
    model.eval()
    validate_data = DataLoader(validate_dataset,batch_size,shuffle=False,drop_last=False,num_workers=6,pin_memory=True)
    total_loss= 0
    for _, (x,a,t,c) in enumerate(validate_data):
        x,a,t,c = x.to(device),a.to(device),t.to(device),c.to(device)
        model.eval()
        if count_data:
            _,_,_,_, _, _, _, _, _, _, loss, _ = model(x,a,t,c)
        else:
            _,_,_,_, _, _, _, loss, _ = model(x,a,t,c)
        total_loss+=loss.item()    
    return loss
...
def inference_model(device, infer_dataset, model, batch_size, count_data=False):
    ...
    Returns:
        ...
        zu: latent representation that is independent from perturbation
        mu_d: mean of latent representation that depends on perturbation
        mu_u: mean of latent representation that is independent from perturbation
        rho: mean expression level
        dispersion: dispersion parameter (only for count data)
        pi: zero-inflation parameter (only for count data)
        library_size: size of the library (only for count data) 
    """
    model = model.to(device) 
    model.eval()
    infer_data = DataLoader(infer_dataset,batch_size,shuffle=False,drop_last=False,num_workers=6,pin_memory=True)
    zd_list = []
    zu_list = []
    mud_list = []
    mui_list = []
    ...
    s_list = []
    if count_data:
        for _, (x,a,t,c) in enumerate(infer_data):
            x,a,t,c = x.to(device),a.to(device),t.to(device),c.to(device)
            model.eval()
            zd,zu,mu_d,mu_u,logvar_d,logvar_u,rho,dispersion, pi,s = model(x,a,t,c,train=False)
            
            zd_list.append(zd.detach().cpu().numpy())
            zu_list.append(zu.detach().cpu().numpy())
            mud_list.append(mu_d.detach().cpu().numpy())
            mui_list.append(mu_u.detach().cpu().numpy())
            logvard_list.append(logvar_d.detach().cpu().numpy())
            logvari_list.append(logvar_u.detach().cpu().numpy())
        ...
        zu = np.concatenate(zu_list, axis=0)
        mu_d = np.concatenate(mud_list, axis=0)
        mu_u = np.concatenate(mui_list, axis=0)
        logvar_d = np.concatenate(logvard_list, axis=0)
        logvar_u = np.concatenate(logvari_list, axis=0)
        ...
        return zd, zu, mu_d, mu_u, logvar_d, logvar_u, rho, dispersion, pi, library_size
    else:
        for _, (x,a,t,c) in enumerate(infer_data):
            x,a,t,c = x.to(device),a.to(device),t.to(device),c.to(device)
            model.eval()
            zd,zu,mu_d,mu_u,logvar_d,logvar_u,rho = model(x,a,t,c,train=False)
        ...
        logvar_u = np.concatenate(logvari_list, axis=0)
        rho = np.concatenate(rho_list, axis=0)
        return zd, zu, mu_d, mu_u, logvar_d, logvar_u, rho
